...
[group('git')]
git-clone repo:
    @if [ "{{repo}}" = "django-mongodb-backend" ]; then \
    ...
    elif [ "{{repo}}" = "langchain-mongodb" ]; then \
        dm repo clone langchain-mongodb --install; \
        dm repo clone pymongo-search-utils --install; \
...
git-remote repo:
    ...
    elif [ "{{repo}}" = "django-mongodb-extensions" ]; then \
        echo "Setting remotes for django-mongodb-extensions"; \
    ...
    elif [ "{{repo}}" = "mongo-python-driver" ]; then \
